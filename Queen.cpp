...
Queen::Queen(PieceColor color, Position pos)
    : Piece(color, PieceType::QUEEN, pos)
{}

string Queen::getSymbol() const {
    return (m_color == PieceColor::WHITE) ? "Q" : "q";
}

vector<Position> Queen::getValidMoves(const Board& board) const {
...
void Queen::draw(sf::RenderWindow& window, float tileSize) const {
