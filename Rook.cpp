...
Rook::Rook(PieceColor color, Position pos)
    : Piece(color, PieceType::ROOK, pos)
{}

string Rook::getSymbol() const {
    return (m_color == PieceColor::WHITE) ? "R" : "r";
}

vector<Position> Rook::getValidMoves(const Board& board) const {
...
void Rook::draw(sf::RenderWindow& window, float tileSize) const {
